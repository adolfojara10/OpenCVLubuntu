...
int main(){
    ...
    string img1 = "/home/computacion/Desktop/OpenCV/A practical introduction to CV/Unit 3 - Histograms/ball.jpg";

    Mat image1 = imread(img1, COLOR_BGR2GRAY), greyImage;
    resize(image1, image1, Size(500,500));

    cvtColor(image1, greyImage,COLOR_RGB2GRAY);
    ...
    calcHist(&greyImage, 1, channelNumbers, Mat(), histogram, 1, &numberBins, &channelRanges);
...
//{} <> []
    ...
    MatND smoothedHistogram = histogram.clone();

    for (int i = 0; i<histogram.rows -1; ++i){
        smoothedHistogram.at<float>(i) = (histogram.at<float>(i-1) + histogram.at<float>(i) + histogram.at<float>(i+1))/3;
    }

    Mat smoothHistImage;
    normalize(smoothedHistogram, smoothedHistogram,0, smoothHistImage.rows, NORM_MINMAX, -1, Mat());

    for (int i=1; i< histsize; i++){

        line(smoothHistImage, Point(binW*(i-1), histH - cvRound(smoothedHistogram.at<float>(i-1))), 
            Point(binW*(i), histH - cvRound(smoothedHistogram.at<float>(i))),
            Scalar(255,0,0),2, 8, 0      
    ...
    //imshow("Smoothed Histogram", smoothHistImage);
    //waitKey();
...
//{} <> []   
 ...
 /*   imshow("IMage", image1);
     ...
     */
//{} <> []   
    string img2 = "/home/computacion/Desktop/OpenCV/A practical introduction to CV/Unit 3 - Histograms/TrinityCampanile3.JPG";

    Mat image2 = imread(img2), imageHLS;
    

    cvtColor(image2, imageHLS, COLOR_RGB2HLS);

    vector<Mat> channelsHLS(imageHLS.channels());
    split(imageHLS, channelsHLS);
    equalizeHist(channelsHLS[1], channelsHLS[1]);
    merge(channelsHLS, imageHLS);

    cvtColor(imageHLS, imageHLS, COLOR_HLS2BGR);

    imshow("IMage", image2);
    imshow("Equalized", imageHLS);
...
//{} <> []   
	...
	string img55 = "/home/computacion/Desktop/OpenCV/A practical introduction to CV/Unit 3 - Histograms/ball.JPG";

    Mat image55 = imread(img55);

	CV_Assert( image55.type() == CV_8UC3 );
	// Populate an n*3 array of float for each of the n pixels in the image
	Mat samples(image55.rows*image55.cols, image55.channels(), CV_32F);
	float* sample = samples.ptr<float>(0);
	for(int row=0; row<image55.rows; row++)
		for(int col=0; col<image55.cols; col++)
			for (int channel=0; channel < image55.channels(); channel++)
				samples.at<float>(row*image55.cols+col,channel) = 
								(uchar) image55.at<Vec3b>(row,col)[channel];
	...
	kmeans(samples, k, labels, TermCriteria(1, 1, 0.0001),
		iterations, KMEANS_PP_CENTERS, centres );
	// Put the relevant cluster centre values into a result image
	Mat& result_image = Mat(image55.size(), image55.type() );
	for(int row=0; row<image55.rows; row++)
		for(int col=0; col<image55.cols; col++)
			for (int channel=0; channel < image55.channels(); channel++)
				result_image.at<Vec3b>(row,col)[channel] = (uchar) centres.at<float>(*(labels.ptr<int>(row*image55.cols+col)), channel); 
